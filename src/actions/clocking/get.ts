...
export async function getClockings(page: number, pageSize: number, searchDate?: string, park?: String, type?: number, status?: number): Promise<{ status: number, data: any, count: number, countExit: number, total_clockings: number, totalScannBusHaveExistedParkAndNotEntredRegion: number, uniqueVehicles: number, uniqueConducteurs: number, scannBusHaveExistedParkAndNotEntredRegion: any[] }> {
    ...
    try {
        ...
        try {
            ...
            const enteredVehicleIds = new Set(enteredOrExistVehiclesFromRegion.map(v => v.vehicle_id));
...
export async function getClockingsVehicleNow(vehicle_id?: string, park?: string, region?: string, date?: string): Promise<{ status: number, data: any }> {
    try {
        ...
        const enteredOrExitVehicleFromRegionIds = new Set(enteredOrExistVehiclesFromRegion.map(v => v.vehicle_id));
        const vehicleWithoutEnteringRegion = exitedVehicleIds.filter(id => !enteredOrExitVehicleFromRegionIds.has(id));
...
export async function getVehiclesExceedingThreshold(
    ...
    try {
        ...
        const results = Array.from(vehicleMap.values())
            .filter((clocking) => {
                const timeDiff = now.getTime() - new Date(clocking.created_at).getTime();
...
export async function getClockingsChefParc(page: number, pageSize: number, searchDate?: string, type?: number, status?: number): Promise<{ status: number, data:{ clockingFormatted:any, message?:string, count: number, countExit: number, total_clockings: number, totalScannBusHaveExistedParkAndNotEntredRegion: number, uniqueVehicles: number, uniqueConducteurs: number, scannBusHaveExistedParkAndNotEntredRegion: any[]} }> {
    const e = await getTranslations('Error');
    try {
        const session = await verifySession();
        if (!session?.data?.user) {
            return { status: 401, data: { message: e("unauthorized") , clockingFormatted:[], count: 0, countExit: 0, total_clockings: 0, totalScannBusHaveExistedParkAndNotEntredRegion: 0, uniqueVehicles: 0, uniqueConducteurs: 0, scannBusHaveExistedParkAndNotEntredRegion: [] }};
        }
        ...
        console.error("An error occurred in getClockings:", error);
        return { status: 500, data: { message: e("error") , clockingFormatted:[], count: 0, countExit: 0, total_clockings: 0, totalScannBusHaveExistedParkAndNotEntredRegion: 0, uniqueVehicles: 0, uniqueConducteurs: 0, scannBusHaveExistedParkAndNotEntredRegion: [] }};
    }
}