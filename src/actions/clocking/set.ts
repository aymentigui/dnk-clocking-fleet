...
export async function createClocking(data: any) {
  ...
  try {
    ...
    if (!existingDevice.park_id && !existingDevice.region_id)
    ...
    if (existingDevice.park_id) {
      ...
      if (!existingPark)
        return { status: 404, data: { message: "موقف الحافلات غير موجود" } };
    }
    ...
    if (existingDevice.region_id) {
      ...
      if (!existingPark)
        return { status: 404, data: { message: "المحطة غير موجودة" } };
    }
    ...
    if (!existingVehicle)
      return { status: 404, data: { message: "الحافلة غير موجودة" } };
    ...
    const status =
      data.type === 3 || data.type === 4
        ? (existingVehicle.region_id &&
          ...
          ? 1
          : 0
        : existingVehicle.park_id &&
            existingDevice.park_id &&
            existingVehicle.park_id == existingDevice.park_id
          ? 1
          : 0;
    ...
    const conducteur_name =
      `${existingConducteur?.firstname ?? ""} ${existingConducteur?.lastname ?? ""}`.trim();
    const vehicle =
      [
        existingVehicle?.matricule,
    ...
    return {
      status: 200,
      data: {
        message: "تم الاستقبال بنجاج",
        conducteur_name: conducteur_name,
        vehicle: vehicle,
      },
    };
  } catch (error) {
    ...
    prisma.notification.create({
      data: {
        ...
        contenu:
          ...
          existingVehicle.matricule +
          "(de parc :" +
          existingVehicle?.park?.name +
          " et parc :" +
          existingVehicle?.region?.name +
          ")" +
          " vient de passer un pointage incorrect" +
          (existingDevice.park
            ? " dans la parc " +
              existingDevice.park.name +
              "(" +
              existingDevice.park.address +
              ")"
            : existingDevice.region
              ? " dans la region " +
                existingDevice.region.name +
                "(" +
                existingDevice.region.address +
                ")"
              : " avec un appareil qui n'a pas de parc et pas de région"),
        user: {
          connect: {
