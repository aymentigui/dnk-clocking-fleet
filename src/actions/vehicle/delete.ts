...
export async function deleteVehicles(ids: string[]): Promise<{ status: number, data: { message: string } }> {
    ...
    try {
        ...
        const hasPermissionAdd = await withAuthorizationPermission(['vehicles_delete'],session.data.user.id);
