...
const actionsCell = (row: any) => {
  ...
  const { openDialog } = useAddUpdateParkDialog();
  const { session } = useSession()
  const hasPermissionDeleteUsers = (session?.user?.permissions.find((permission: string) => permission === "park_delete") ?? false) || session?.user?.is_admin;
  const hasPermissionUpdateUsers = (session?.user?.permissions.find((permission: string) => permission === "park_update") ?? false) || session?.user?.is_admin;
